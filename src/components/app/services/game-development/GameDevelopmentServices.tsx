...
export function GameDevelopmentServices() {
  return (
    ...
    >
      ...
      <p className="text-base whitespace-pre-line text-white md:text-lg">
        {GAME_DEV_SERVICES.desc}
      </p>
      <div className="grid grid-cols-1 gap-5 px-10 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 lg:px-20">
        {GAME_DEV_SERVICES.services.map((service, index: number) => {
          return (
            <div
              key={index}
              className="group hover:bg-primary/50 border-primary/50 relative w-full overflow-hidden rounded-xl border-[1px] p-6 px-10 transition-colors duration-500 hover:border-white/20 hover:text-white"
            >
              ...
              <div className="relative z-10 flex h-full flex-col items-center gap-4 md:items-start">
                <h1 className="text-center text-xl font-bold text-white sm:text-2xl md:text-start">
                  {service.heading}
                </h1>
