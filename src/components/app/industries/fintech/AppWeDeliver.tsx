...
import { FINTECH_APPS_SECTION } from "@/data/industries/fintech.data";
import { useState } from "react";

export function AppWeDeliver() {
  const [activeIndex, setActiveIndex] = useState(0);
  const ICon2 = FINTECH_APPS_SECTION.apps[activeIndex].icon;
  return (
    <div className="space-y-5 px-5 py-5 md:space-y-10 md:px-20 md:py-10">
      <h1 className="text-center text-2xl sm:text-3xl leading-snug font-bold whitespace-pre-line md:text-5xl">
        {FINTECH_APPS_SECTION.headingParts.map((part, i) => (
      ...
      <p className={"pt-5 text-center text-xl"}>
        {FINTECH_APPS_SECTION.description}
      </p>
      ...
      <div className="md:hidden">
        <Accordion
          ...
          className="space-y-4"
        >
          {FINTECH_APPS_SECTION.apps.map((app, i) => {
            const Icon = app.icon;
            return (
              <AccordionItem
                key={i}
      ...
      <div className="relative hidden gap-4 md:flex">
        <div className={"flex flex-1 flex-col gap-8"}>
            ...
            {FINTECH_APPS_SECTION.apps.map((app, i) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "shadow-primary/40 flex h-full flex-col text-primaryCo items-center gap-2 rounded-xl border-2 border-dashed border-gray-400 px-3 py-4 text-xl font-bold transition-colors duration-300 lg:flex-row lg:px-6 lg:py-8",
                    activeIndex === i &&
          ...
          <div className="flex flex-col items-start justify-around gap-4 rounded-xl bg-white p-10">
            ...
            <h1 className="text-primary text-4xl font-extrabold">
              {FINTECH_APPS_SECTION.apps[activeIndex].title}
            </h1>
            <p className="text-xl font-medium">
              {FINTECH_APPS_SECTION.apps[activeIndex].desc}
            </p>
          </div>
        </div>
        <div className={"hidden w-2/5 lg:flex"}>
          <div className="] right-0 bottom-0 flex overflow-hidden rounded-xl md:h-[400px] lg:absolute lg:h-[700px]">
            <Image
              src={FINTECH_APPS_SECTION.apps[activeIndex].img}
              alt="AR/VR Solution"
              width={500}
              height={600}
