      ...
      >
        ...
        >
          <h1 className="text-center text-3xl leading-snug md:text-start md:text-5xl">
            {ECOMMERCE_HERO_SECTION.headingParts.map((part, i) => (
              <span key={i} className={part.className}>
                {part.text}{" "}
              </span>
            ))}
          </h1>
          <p className={"text-center md:text-start md:text-xm"}>
            {ECOMMERCE_HERO_SECTION.content}
          </p>
          ...
          <CTAButton2
            href={ECOMMERCE_HERO_SECTION.CTABtnLink}
            className={"self-start"}
            label={ECOMMERCE_HERO_SECTION.CTAButton}
          />
        </div>
        <div className="w-full max-w-md lg:w-1/2 lg:max-w-2/5">
          <Image
            src={ECOMMERCE_HERO_SECTION.bannerImg}
            height={200}
            width={500}
